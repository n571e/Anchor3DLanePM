_base_ = []
custom_imports = dict(imports=['projects.bevformer', 'projects.topopoint'])
...
data_root = '/data/fuyanping/data/OpenLane-V2/'
...
model = dict(
    ...
    lane_head=dict(
        ...
        transformer=dict(
            type='TopoPointTransformerDecoderOnly',
            embed_dims=_dim_,
            pts_dim=pts_dim,
            decoder=dict(
                type='TopoPointSGNNDecoder',
                pc_range=point_cloud_range,
                num_layers=6,
                return_intermediate=True,
                transformerlayers=dict(
                    ...
                    operation_order=('self_attn', 'norm','norm', 'cross_attn','cross_attn', 'norm','norm','ffn','norm','norm'),
                                     ))),
...
train_pipeline = [
    ...
    dict(type='CustomCollect3D', keys=[
        'img', 'front_img', 'gt_lanes_3d', 'gt_lane_labels_3d', 'gt_lane_adj',
...
test_pipeline = [
    ...
    dict(type='PadMultiViewImageSame2Max', size_divisor=32),
    dict(type='CustomFormatBundle3DLane', class_names=class_names),
    dict(type='CustomCollect3D', keys=['img', 'front_img'])
]
