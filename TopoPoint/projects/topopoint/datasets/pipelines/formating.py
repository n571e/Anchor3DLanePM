    ...
    def __call__(self, results):
        ...
        Returns:
            dict: The result dict contains the data that is formatted with
                default bundle.
        """
        if 'front_img' in results:
            if isinstance(results['front_img'], list):
                # process multiple imgs in single frame
                front_imgs = [front_img.transpose(2, 0, 1) for front_img in results['front_img']]
                front_imgs = np.ascontiguousarray(np.stack(front_imgs, axis=0))
                results['front_img'] = DC(to_tensor(front_imgs), stack=True)
            else:
                front_img = np.ascontiguousarray(results['front_img'].transpose(2, 0, 1))
                results['front_img'] = DC(to_tensor(front_img), stack=True)        
