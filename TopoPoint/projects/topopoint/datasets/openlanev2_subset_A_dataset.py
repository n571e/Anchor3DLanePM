...
class OpenLaneV2_subset_A_Dataset(Custom3DDataset):
    ...
    def union2one(self, queue):
        ...
        front_imgs_list = [each['front_img'].data for each in queue]
        metas_map = {}
        prev_pos = None
        prev_angle = None
        ...
        queue[-1]['front_img'] = DC(torch.stack(front_imgs_list),
                              cpu_only=False, stack=True)
    ...
    def format_openlanev2_gt(self):
        ...
        for idx in range(len(self.data_infos)):
            ...
            for lane in info['annotation']['lane_centerline']:
                ...
                end_pt.append(lane['points'][0])
                end_pt.append(lane['points'][-1])


            end_pt = torch.Tensor(np.array(end_pt))
            
            end_pt,pts_idx = torch.unique(end_pt,dim=0,sorted=False,return_inverse=True)
            # counts = torch.bincount(pts_idx)
            # unique_idx = torch.nonzero(counts > 1).flatten()

            # end_pt = end_pt[unique_idx]
            inter_points = []
            for pt in end_pt:
                inter_points.append({'points': pt[None,:].numpy()})

            info['annotation']['inter_points'] = inter_points
    ...
    def format_results(self, results, jsonfile_prefix=None):
        ...
        for idx, result in enumerate(results):
            ...
            if result['pts_results'] is not None:
                pts_results = result['pts_results']

                scores = pts_results[1]
                # from IPython import embed; embed()
                pts_valid_indices = np.argsort(-scores)
                pts = pts_results[0][pts_valid_indices]
                pts = pts.reshape(-1, pts.shape[-1] // 3, 3)

                
                scores = scores[pts_valid_indices]
                for pred_idx, (pts, score) in enumerate(zip(pts, scores)):
                    points = pts
                    pts_info = dict(
                        id = 10000 + pred_idx,
                        points = points.astype(np.float32),
                    ...
                    pred_info['inter_points'].append(pts_info)
    ...
    def evaluate(self, results, logger=None, show=False, out_dir=None, **kwargs):
        ...
        metric_results = {
            ...
            'DET_p': metric_results['OpenLane-V2 Score']['DET_p'],
            'TOP_ll': metric_results['OpenLane-V2 Score']['TOP_ll'],
            'TOP_lt': metric_results['OpenLane-V2 Score']['TOP_lt'],
        }
        return metric_results

    def show(self, results, out_dir, score_thr=0.3, show_num=500, **kwargs):
        """Show the results.
