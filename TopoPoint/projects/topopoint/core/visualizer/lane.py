...
def show_bev_results(gt_lane, pred_lane, gt_lclc=None, pred_lclc=None, pred_pts=None, only=None, map_size=[-55, 55, -30, 30], scale=10):
    ...
    if pred_pts is not None:
        ...
        for pts in pred_pts:
            draw_coor = (scale * (-pts[:, :2] + np.array([map_size[1], map_size[3]]))).astype(int)
            image = cv2.polylines(image, [draw_coor[:, [1,0]]], False, PTS_PRED_COLOR, max(round(scale * 0.2), 1))
            image = cv2.circle(image, (draw_coor[0, 1], draw_coor[0, 0]), max(2, round(scale * 0.5)), PTS_PRED_COLOR, -1)
            image = cv2.circle(image, (draw_coor[-1, 1], draw_coor[-1, 0]), max(2, round(scale * 0.5)) , PTS_PRED_COLOR, -1)
